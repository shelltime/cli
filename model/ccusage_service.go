...
func NewCCUsageService(config ShellTimeConfig, cmdService CommandService) CCUsageService {
...
func (s *ccUsageService) collectData(ctx context.Context, since time.Time) (*CCUsageData, error) {
	// Check if bunx exists using command service lookPath that checks common installation locations
	bunxPath, bunxErr := s.commandService.LookPath("bunx")
	npxPath, npxErr := s.commandService.LookPath("npx")
